...
class NewsPost(models.Model):
    ...
    slug=AutoSlugField(max_length=200, populate_from='post_title',unique=True,default=None,
    always_update=False,
    editable=True)
    ...
    def compress_and_resize(self):
        """
        Aggressively compress image until it becomes <100 KB.
        """
        ...
        # Convert PNG/WebP → RGB
        if img.mode in ("RGBA", "P"):
            img = img.convert("RGB")

        MAX_SIZE = 100 * 1024

        # Progressive dimension reductions
        target_widths = [1280, 1100, 900, 700, 500, 350, 250]
        ...
        for width in target_widths:
            ...
            # ---- Try compressing with quality loop ----
            for quality in range(85, 10, -5):
                ...
                if size <= MAX_SIZE:
                    # SUCCESS → Save compressed version
                    compressed_name = (
                        os.path.splitext(os.path.basename(image_path))[0] + "_c.jpg"
                    )
        ...
        # Final fallback (guaranteed <100 KB)
        buffer = BytesIO()
    ...
    def save(self, *args, **kwargs):
        # Step 1: Upload + ImageCropField cropping
        super().save(*args, **kwargs)

        # Step 2: Compress AFTER cropping is completed
        self.compress_and_resize()

        # Step 3: Save updated compressed image only
